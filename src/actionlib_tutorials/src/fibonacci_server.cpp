...
void FibonacciAction::executeCB(const actionlib_tutorials::FibonacciGoalConstPtr &goal) 
{
    ...
    ROS_INFO("%s: Executing, creating fibonacci sequence of order %d with seeds %d, %d", action_name_.c_str(), goal->order, feedback_.sequence[0], feedback_.sequence[1]);

    // start executing the action
    for (int i = 1; i <= goal->order; i++) {
        // check that preempt has not been requested by the vlient
